...
public class PlayerSkeleton {
	...
	public static final boolean RUN_EVO_WITH_LOOKAHEAD = true;
	...
	public static class gameThread implements Callable<Integer> {
		...
		public Integer call() {
			...
			try {
				rowsCleared = RunAI(pEvolve[guy], 300, Integer.MAX_VALUE, false, RUN_EVO_WITH_LOOKAHEAD);
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public static double[] RunEvolution(int heuristicsCount){
		...
		while(averageScore < Integer.MAX_VALUE - 1 && generationCount < 50) {
			...
			averageScore = 0;
			for (int j = 0; j < 3; j++) {
				averageScore += RunAI(finalParameters, 20, Integer.MAX_VALUE, false, RUN_EVO_WITH_LOOKAHEAD);
			}
			averageScore /= 3;
